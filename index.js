...
async function updateAllRepos() {
	try {
		...
		const repos = res.data.filter(r => r.name !== "libialany" && r.name !== "projects-readme" && r.name !== "readme-github-actions");
...
async function updateReadMe(repo) {
	try {
		const res = await client.request(`GET /repos/libialany/${repo}/contents/README.md`);
		...
		try {
			const content = `\n${getNewProjectSection()}`;
			await client.request(`PUT /repos/libialany/${repo}/contents/README.md`, {
...
async function commitNewReadme(repo, path, sha, encoding, updatedContent) {
	try {
		await client.request(`PUT /repos/libialany/${repo}/contents/{path}`, {
